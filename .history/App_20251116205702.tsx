...
export default function App() {
    ...
    useEffect(() => {
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(
            (event, session) => {
                console.log("🔔 Trạng thái đăng nhập thay đổi:", event);
                // Chỉ set isLoggedIn = true khi user SIGNED_IN (login thành công)
                // Tránh trigger khi SIGNED_UP (vì SignUpScreen sẽ logout sau signup)
                if (event === "SIGNED_IN") {
                    setIsLoggedIn(true);
                } else if (event === "SIGNED_OUT") {
                    setIsLoggedIn(false);
                }
                // Bỏ qua các event khác như SIGNED_UP, USER_UPDATED, ...
            }
        );
