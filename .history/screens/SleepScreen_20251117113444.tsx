...
export default function SleepScreen() {
    ...
    const saveTime = async () => {
        if (!userId) {
            Alert.alert("Lỗi", "User ID không được load");
            return;
        }
        ...
        try {
            ...
            console.log("Saving time:", { newBed, newWake, editingTime, userId });
            ...
            if (!result.success) {
                setTimeout(() => {
                    ...
                    Alert.alert("Cảnh báo", "Không lưu được trên server: " + (result.error || "Lỗi không xác định"));
                }, 200);
            }
        } catch (e: any) {
        ...
        >
            ...
            <View style={styles.barChartContainer}>
                {records.map((record, i) => {
                    // Tính chiều cao cột
                    const barHeight = (record.sleep_hours / maxSleep) * 100;
...
const styles = StyleSheet.create({
    ...
    barForeground: {
        width: "100%",
        backgroundColor: "#00D4FF", // Cyan rực rỡ như hình
        borderRadius: 8,
    },
    barSelected: {
    ...
    // Modal - Cải thiện giao diện theo màu chủ đạo
    modalBg: {
        flex: 1,
    ...
    pickerBtn: { padding: 14, borderRadius: 10, margin: 4 },
    pickerVal: {
        fontSize: 48,
        fontWeight: "900",
        color: "#5865F2",
