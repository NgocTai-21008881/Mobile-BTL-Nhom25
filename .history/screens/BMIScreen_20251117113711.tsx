...
export default function BMIScreen() {
    ...
    const loadBMIData = async () => {
        setLoading(true);
        try {
            // Lấy dữ liệu hoạt động (chứa BMI nếu có)
            const activities = await fetchDailyActivity(userId!, "week");
            
            // Nếu không có BMI data, tạo dữ liệu mẫu
            const mockBMI = activities.map((a, i) => ({
                date: a.date,
    ...
    const saveBMI = async () => {
        ...
        setIsSaving(true);
        const bmi = parseFloat(calculateBMI(parseFloat(weight), parseFloat(height)));

        try {
            // Lưu BMI vào database
            ...
            if (error) {
                ...
                Alert.alert("Lỗi", "Không lưu được BMI");
                return;
            }
    ...
    const avgBMI = useMemo(
        () => (bmiRecords.length > 0
            ? (bmiRecords.reduce((a, b) => a + b.bmi, 0) / bmiRecords.length).toFixed(1)
            : "0"),
        [bmiRecords]
    );
