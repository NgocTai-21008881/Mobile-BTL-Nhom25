...
export default function CaloriesScreen() {
    ...
    const saveCustomCalories = async () => {
        ...
        if (!userId) {
            Alert.alert("Lỗi", "Chưa xác thực người dùng");
            return;
        }
        ...
        try {
            ...
            if (existingRecord) {
                ...
                const { error: updateError } = await supabase
                    .from("daily_activity")
                    .update({ calories: totalCals })
                    .eq("id", existingRecord.id);

                if (updateError) throw updateError;
            } else {
                // Insert record mới nếu chưa tồn tại
            ...
            const newCalories = [...caloriesData];
            newCalories[newCalories.length - 1] = totalCals;
            ...
            Alert.alert("Thành công", `Đã lưu ${totalCals} kcal`);
        } catch (e) {
            console.error("Lỗi lưu calo:", e);
            Alert.alert("Lỗi", "Không thể lưu calo, vui lòng thử lại");
        }
    };
