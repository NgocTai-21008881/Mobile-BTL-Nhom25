...
export default function BlogDetailScreen() {
    ...
    const [activeTab, setActiveTab] = useState<"content" | "comments" | "ratings">("content");
        ...
        Alert.alert("Xác nhận", "Bạn có chắc muốn xóa đánh giá này?", [
            { text: "Hủy", onPress: () => {} },
            {
                ...
                onPress: () => {
                    ...
                    setRatings(updated);
                    if (updated.length > 0) {
                        const avg = (updated.reduce((sum, r) => sum + r.rating, 0) / updated.length).toFixed(1);
                        setAverageRating(parseFloat(avg));
                    }
                },
                style: "destructive",
        ...
        <>
            ...
            <View style={styles.contentArticle}>
                ...
                <Text style={styles.contentText}>
                    {content || `Bài viết chi tiết về chủ đề ${title || "Dinh dưỡng"}.`}
                </Text>
            </View>
    ...
    const renderRatingsTab = () => (
        <View style={styles.tabContent}>
            ...
            {ratings.length > 0 && (
                <>
                    <Text style={styles.ratingsListTitle}>Đánh giá mới nhất</Text>
                    <FlatList
                        data={ratings}
                        keyExtractor={(item) => item.id}
                        scrollEnabled={false}
                            ...
                            <View style={styles.ratingCard}>
                                <View style={styles.ratingCardHeader}>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        ...
                                        {renderStars(item.rating, undefined, 14)}
                                        <Text style={styles.commentTime}>
                                            {item.timestamp}
                                        </Text>
                                    </View>
