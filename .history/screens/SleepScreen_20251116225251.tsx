...
export default function SleepScreen() {
    ...
    const [wakeUpTime, setWakeUpTime] = useState<string>("07:30");
    ...
    const avgSleep = useMemo(() => {
        ...
        return (total / sleepData.length).toFixed(2);
    }, [sleepData]);

    const sleepRate = useMemo(() => {
        const avg = parseFloat(avgSleep);
        if (avg < 5) return "Poor";
        if (avg < 7) return "Fair";
        return "Good";
    }, [avgSleep]);

    const calculateDeepSleep = useMemo(() => {
        const avg = parseFloat(avgSleep);
        const deepSleepPercent = Math.min(85, Math.round((avg / 9) * 85));
        const avgDeepTime = Math.round((parseFloat(avgSleep) * 0.6 * 60) / 60 * 60);
    ...
    const calculateScheduleSleep = useMemo(() => {
    ...
    return (
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.averageCard}>
                <Text style={styles.averageLabel}>Your average time of sleep a day is</Text>
                    ...
                    {sleepData.map((item, index) => (
                        <View key={index} style={styles.barWrapper}>
                            <View
                                style={[
                                    styles.bar,
                                    { height: Math.max(10, (item.sleep_hours / 10) * 140) },
                                ]}
                            />
            ...
            <View style={styles.statsContainer}>
                <View style={styles.statCard}>
                    <View style={styles.statHeader}>
                        <MaterialCommunityIcons name="sleeping" size={24} color="#00BCD4" />
                ...
                <View style={styles.statCard}>
                    <View style={styles.statHeader}>
                        <MaterialCommunityIcons name="moon-stars" size={24} color="#FF9800" />
                        <Text style={styles.statTitle}>Deepsleep</Text>
                    </View>
                    <Text style={styles.statValue}>{calculateDeepSleep.time}</Text>
                </View>
            </View>
            ...
            <View style={styles.scheduleSection}>
                ...
                <View style={styles.timeButtonsContainer}>
                    ...
                    >
                        <MaterialCommunityIcons name="bed-time" size={28} color="#fff" />
                    ...
                    >
                        <MaterialCommunityIcons name="alarm" size={28} color="#fff" />
                        ...
                        <View style={styles.timePicker}>
                            <View style={styles.timeInputContainer}>
                                ...
                                <View style={styles.timeInputBox}>
                                    <TouchableOpacity onPress={() => {
                                        const [h, m] = (editingTime === "bedtime" ? bedtime : wakeUpTime).split(":").map(Number);
                                        handleTimeChange(h === 0 ? 23 : h - 1, m);
                                    }}>
                                        <AntDesign name="minus" size={24} color="#00BCD4" />
                                    </TouchableOpacity>
                                    <Text style={styles.timeInputValue}>
                                        {String((editingTime === "bedtime" ? bedtime : wakeUpTime).split(":")[0]).padStart(2, "0")}
                                    </Text>
                                    <TouchableOpacity onPress={() => {
                                        const [h, m] = (editingTime === "bedtime" ? bedtime : wakeUpTime).split(":").map(Number);
                                        handleTimeChange(h === 23 ? 0 : h + 1, m);
                                    }}>
                                        <AntDesign name="plus" size={24} color="#00BCD4" />
                                    </TouchableOpacity>
                                </View>
                            </View>
                            ...
                            <View style={styles.timeInputContainer}>
                                ...
                                <View style={styles.timeInputBox}>
                                    <TouchableOpacity onPress={() => {
                                        const [h, m] = (editingTime === "bedtime" ? bedtime : wakeUpTime).split(":").map(Number);
                                        handleTimeChange(h, m === 0 ? 59 : m - 1);
                                    }}>
                                        <AntDesign name="minus" size={24} color="#00BCD4" />
                                    </TouchableOpacity>
                                    <Text style={styles.timeInputValue}>
                                        {String((editingTime === "bedtime" ? bedtime : wakeUpTime).split(":")[1]).padStart(2, "0")}
                                    </Text>
                                    <TouchableOpacity onPress={() => {
                                        const [h, m] = (editingTime === "bedtime" ? bedtime : wakeUpTime).split(":").map(Number);
                                        handleTimeChange(h, m === 59 ? 0 : m + 1);
                                    }}>
                                        <AntDesign name="plus" size={24} color="#00BCD4" />
                                    </TouchableOpacity>
                                </View>
