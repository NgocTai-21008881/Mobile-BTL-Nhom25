...
export default function DoubleSupportScreen() {
    ...
    const handleSetGoal = () => {
        ...
        Alert.alert("✅ Thành công", `Mục tiêu đã được cập nhật thành ${newGoal.toLocaleString('vi-VN')} bước`);
    };

    // Handle share
    const handleShare = () => {
        const message = `📊 Tôi đã đạt trung bình ${avg.toLocaleString('vi-VN')} bước trong ${selectedRange === 'week' ? 'tuần' : selectedRange === 'month' ? 'tháng' : 'quý'} này! 🎉`;
